...
-- comm_pct
select avg(comm_pct),sum(comm_pct)/35, sum(comm_pct)/107 ,avg(ifnull(comm_pct,0)) from emp;
...
select max(comm_pct)'가장큰커미션비율',
	   min(comm_pct)'가장작은커미션비율'
from emp;
-- TODO:  커미션 비율(comm_pct)의 평균을 조회. 소수점 이하 2자리까지 출력

select round(avg(ifnull(comm_pct,0)),2)
from emp;
...
from emp
group by dept_name
having count(*) >= 10; -- 이 집계 결과 조건을 만족하는 그룹만 Filtering 한다.
...
if(grouping(year(hire_date))=1, '총계', year(hire_date))"입사년도",
	   count(*)'직원수',
